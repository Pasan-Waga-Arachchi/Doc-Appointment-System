...
namespace RAD_Project
{
    public partial class UserControl1_Channel : UserControl
    {
        ...
        private void btn_show_doctors_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string query = "SELECT Name,Specialization,Time,Date FROM Doctor WHERE Specialization = '" + state + "' AND Date= '" + date + "' AND Time= '" + time + "'";
                {
                    try
                    {
        ...
        private void btn_appoint_Click_Click(object sender, EventArgs e)
        {
            SqlConnection connection = DatabaseConnection.Instance.GetConnection();

            if (dataGridView1.SelectedRows.Count > 0)
            {
                string doctorName = dataGridView1.SelectedRows[0].Cells["Name"].Value.ToString();
                ...
                try
                {
                    string query = "INSERT INTO Appointment (Appointment_ID, Patient_Name, Patient_ID,Name,Appointment_Date,Appointment_Time, Appointment_Reason,Appoinment_Type) " +
                           "VALUES (@AppointmentID,@PatientName,@PatientID,@DoctorName,@AppointmentDate,@AppointmenTime,@Reason,@AppointmentType)";
        ...
        private void btn_appoint_Click_MouseEnter(object sender, EventArgs e)
        {
            btn_show_doctors.BackColor = Color.Lime;
        }
        ...
        private void btn_appoint_Click_MouseLeave(object sender, EventArgs e)
        {
            btn_show_doctors.BackColor = Color.Aqua;
        }
